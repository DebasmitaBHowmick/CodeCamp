...
import { setSortBy, setBestSellerOnly, setRatings, setInStock, clearFilter } from "../../redux/FilterSlice";


const ProductFilter = ({setToggle}) => {

const dispatch = useDispatch();
const sortBy = useSelector((state) => state.filter.sortBy);
const bestSeller = useSelector(state => state.filter.bestSellerOnly);
const ratings = useSelector(state => state.filter.ratings);
                ...
                <ul className="text-slate-700 dark:text-slate-100">
                  <li className="mt-1 mb-5">
                    ...
                    <div className="flex items-center my-1">
                        <input id="price-sort-1" type="radio" checked={sortBy === "lowtohigh"} onChange={()=> dispatch(setSortBy("lowtohigh"))}name="price-sort" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
                        <label htmlFor="price-sort-1" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Price - Low to High</label>
                    </div>
                    <div className="flex items-center my-1">
                        <input checked={sortBy === "hightolow"}id="price-sort-2" type="radio" onChange={() => dispatch(setSortBy("hightolow"))} name="price-sort" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
                          ...
                          {[4,3,2,1].map((star) => (
                                <div key={star} className="flex items-center my-1">
                                  <input
                                    ...
                                    onChange={() => dispatch(setRatings(star))}
                                    className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 dark:bg-gray-700 dark:border-gray-600"
                                  />
                                  <label
                                    htmlFor={`rating-sort-${star}`}
                  ...
                  <li className="mt-1 mb-5">
                    ...
                    <div className="flex items-center my-1">
                        <input id="best-seller" type="checkbox" value="" checked={bestSeller} onChange={()=> dispatch(setBestSellerOnly())}className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
                        <label htmlFor="best-seller" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">Best Seller Only</label>
                    </div>
                    <div className="flex items-center my-1">
                        <input id="only-instock" type="checkbox" checked={inStock} onChange={()=> dispatch(setInStock())}className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 dark:bg-gray-700 dark:border-gray-600" />
