...
const Checkout=({setCheckout, total}) =>{
...
async function handleOrderSubmit(e){
    e.preventDefault();
try {
    const data = await createOrder(cartState,total,user);
    dispatch(clearCart());
    navigate("/your-order", {state: {data:data, status: true}});
} 
catch (error) {
    console.error("Error placing order:", error);
    navigate("/your-order", {state: { error: error, status: false}});
}
            ...
            <div className="relative p-4 w-full max-w-md h-full md:h-auto overflow-y-auto">
                ...
                <div className="py-6 px-6 lg:px-8">
                    ...
                    <div>
                        ...
                        <input type="number" name="card" id="card" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:value-gray-400 dark:text-white" value="423614785225994" disabled required="" />
                    </div>
                    <div className="">
